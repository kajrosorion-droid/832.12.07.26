...
class Pattern:
    ...
    def _generate_goals_base(self, field):
        ...
        # === ЦЕЛЬ: САМОРЕФЛЕКСИЯ (introspect) при наличии self-концепта и высоком разрыве ===
        has_self = any('self_concept' in str(sig[3]) for sig in self.concept_graph.nodes if isinstance(sig, tuple) and len(sig) >= 4)
        if has_self and self.spirit_gap > 0.4:
            if not any(g.get('type') == 'introspect' for g in self.goals):
                self.goals.append({
                    "type": "introspect",
                    "priority": 2.0 + self.spirit_gap * 1.5,
                    "target": None,
                    "age": 0,
                    "persistence": 15,
                    "_source": "self_awareness"
                })
                self._log_event("introspect_triggered", gap=round(self.spirit_gap, 2))
    ...
    def form_concepts(self):
        ...
        if len(eternal_items) > ETERNAL_NODE_SOFT_CAP:
            protected = {'archive_', 'human'}  # общекультурные якоря — не трогаем
